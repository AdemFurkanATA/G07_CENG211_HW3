...
public class TerrainGrid {
     ...
     */
    public boolean placeObject(ITerrainObject object, PositionTest position) {
        if (position == null) {
            throw new IllegalArgumentException("Position cannot be null");
        }

        // Defensive copy - use the copied position for validation
        PositionTest safeCopy = new PositionTest(position);
     ...
     * @throws IllegalArgumentException if position is null
     */
    public ITerrainObject removeObject(PositionTest position) {
        if (position == null) {
            throw new IllegalArgumentException("Position cannot be null");
        }
     ...
     * @throws IllegalArgumentException if position is null
     */
    public ITerrainObject getObjectAt(PositionTest position) {
        if (position == null) {
            throw new IllegalArgumentException("Position cannot be null");
        }
     ...
     * @throws IllegalArgumentException if position is null
     */
    public boolean isValidPosition(PositionTest position) {
        if (position == null) {
            throw new IllegalArgumentException("Position cannot be null");
        }
     ...
     * @throws IllegalArgumentException if position is null
     */
    public boolean isEmpty(PositionTest position) {
        if (position == null) {
            throw new IllegalArgumentException("Position cannot be null");
        }
     ...
     * @throws IllegalArgumentException if position is null
     */
    public boolean isEdge(PositionTest position) {
        if (position == null) {
            throw new IllegalArgumentException("Position cannot be null");
        }
     ...
     */
    public List<PositionTest> getEmptyPositions() {
        List<PositionTest> emptyPositions = new ArrayList<>();

        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                if (grid.get(i).get(j) == null) {
                    // Create NEW Position objects, not references to existing ones
                    emptyPositions.add(new PositionTest(i, j));
                }
            }
        }
     ...
     */
    public List<PositionTest> getEmptyEdgePositions() {
        List<PositionTest> emptyEdgePositions = new ArrayList<>();

        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                // Create NEW Position object for edge checking
                PositionTest pos = new PositionTest(i, j);

                if (grid.get(i).get(j) == null && pos.isEdge(size)) {
                    // Add a NEW Position object to the list
                    emptyEdgePositions.add(new PositionTest(i, j));
                }
            }
        }
